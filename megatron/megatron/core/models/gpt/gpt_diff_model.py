...
# ($) import subtokenizer
from subtokenizer.layers import BasebLayer, HCLayer
import copy
import pdb

class GPTDiffModel(LanguageModule):
    ...
    def __init__(
        ...
        if self.pre_process or self.mtp_process:
            ...
            self.embedding = LanguageModelEmbedding(
                config=emb_config, # ($)
                vocab_size=(self.base+1) * self.target_length, # ($)
    ...
    def _preprocess(
        ...
        if decoder_input is not None:
            ...
            offsets = table_ids * self.base
            input_ids = input_ids + offsets.unsqueeze(0)
            
            decoder_input = self.embedding(input_ids=input_ids, position_ids=position_ids)

            # ($) sub-tokenization post-processing
            N, B, D = decoder_input.shape
            l = self.target_length
            if self.cat_emb:
                decoder_input = decoder_input.contiguous().view(N // l, l, B, D).permute(0, 2, 1, 3).reshape(N // l, B, l * D)    # [N/l, B, l*D]
            else:
                decoder_input = decoder_input.contiguous().view(N // l, l, B, D).sum(dim=1) # [N/l, l, B, D]
        else:
