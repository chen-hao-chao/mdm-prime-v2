...
def parse_args():
    ...
    parse.add_argument('--gpu_num', type=int, default=4, help='number of gpus')
    ...
    parse.add_argument('--wandb_project', type=str, default='scaling', help='wandb project name')
    parse.add_argument('--data_path', type=str, default='/home/chchao0/scratch/slim_star_combined', help='data path')
    args = parse.parse_args()
    return args

args = parse_args()
model_name = f'Diff_LLaMA_{args.model}M'  # config
run_name = f'mdm-{args.model}M-{args.flops}-ssl-{args.ssl_ratio}' if args.run_name is None else args.run_name
...
def run_zeroshot_eval(fabric, model, tokenizer, step_count):
    ...
    # 2. Run Evaluation
    # Select tasks (e.g., 'hellaswag', 'piqa', 'arc_easy')
    results = simple_evaluate(
        model=harness,
        tasks=["hellaswag", "arc_easy"], # Add your desired tasks here
        batch_size=args.eval_batch_size,
...
def main(fabric, train_data_dir, val_data_dir, resume):
    ...
    train(fabric, state, train_dataloader, val_dataloader, monitor, resume, tokenizer=tokenizer)
    fabric.print(f"Training time: {(time.perf_counter()-train_time):.2f}s")
    if fabric.device.type == "cuda":
        fabric.print(f"Memory used: {torch.cuda.max_memory_allocated() / 1e9:.02f} GB")


def train(fabric, state, train_dataloader, val_dataloader, monitor, resume, tokenizer):
    model = state["model"]
    ...
    for train_data in train_dataloader:
        ...
        if not is_accumulating and (state["step_count"] % eval_step_interval == 0 or state["step_count"] == max_step):
            ...
            # ($)
            run_zeroshot_eval(fabric, model, tokenizer, state["step_count"])
            model.train()
